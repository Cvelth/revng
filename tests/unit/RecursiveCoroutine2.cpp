...
// Test that the coroutine is destroyed before its result can be accessed in a
...
// Test that the coroutine is destroyed before its result can be accessed after
// being awaited.
...
// Test that no moved-from copy of the coroutine result exists when it is
// observed by a non-coroutine caller.
...
// Test that no moved-from copy of the coroutine result exists when it is
// observed after being awaited.
BOOST_AUTO_TEST_CASE(RecursiveCoroutineEvaluationOrder4) {
  int Counter = 0;
  ...
  auto H = [&]() -> RecursiveCoroutine<void> { G(rc_recur F()); };
