...
std::string yield::svg::callGraph(const yield::CrossRelations &Relations,
  ...
  auto InternalGraph = calls::calleePreservingSlice(Result);
...
std::string yield::svg::callGraphSlice(const MetaAddress &SlicePoint,
  ...
  auto ForwardsGraph = calls::calleePreservingSlice(Relations.toYieldGraph(),
                                                    SlicePoint);
  ...
  auto BackwardsGraph = calls::callerPreservingSlice(Relations.toYieldGraph(),
                                                     SlicePoint);
