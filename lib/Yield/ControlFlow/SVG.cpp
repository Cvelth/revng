...
static std::string content(const yield::Node *Node,
  ...
  if (Node->Address.isValid()) {
    ...
    // An error node.
    return R"(<span class="error">Error</span>)";
  }
}
...
static std::string node(const yield::Node *Node,
  ...
  auto Text = llvm::formatv(templates::ForeignObject,
                            ...
                            Node->Size.W,
                            Node->Size.H,
                            content(Node, Function, Binary, IsEntryNode));
  ...
  return llvm::formatv(templates::Node, std::move(Body), std::move(Text));
}
...
static std::string exportCFG(const yield::Graph &Graph,
  ...
  for (const auto *From : Graph.nodes()) {
    for (const auto [To, Edge] : From->successor_edges()) {
      ...
      if (!Edge->Path.empty()) {
        ...
        Result += edge(Edge->Path, Edge->Type, Configuration.UseOrtogonalBends);
      }
    }
  }
  ...
  for (const auto *Node : Graph.nodes()) {
    bool IsEntry = Graph.getEntryNode() == Node;
    Result += node(Node, Function, Binary, Configuration, IsEntry);
  }
