...
static std::string targets(const yield::BasicBlock &BasicBlock,
  ...
  if (BasicBlock.Targets.size() == 1) {
    ...
    revng_assert(BasicBlock.Targets.begin()->Destination.isValid());
    Result += comment(BasicBlock,
                      "always goes to "
                        + targetLink(BasicBlock.Targets.begin()->Destination,
                                     BasicBlock,
                                     Metadata,
                                     Binary));
  } else {
    ...
    for (size_t TargetCounter = 0; const auto &[Dest, _] : BasicBlock.Targets) {
      if (Dest.isValid()) {
        std::string Link = targetLink(Dest, BasicBlock, Metadata, Binary);
