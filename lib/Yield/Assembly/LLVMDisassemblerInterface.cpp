...
yield::Instruction DI::parse(const llvm::MCInst &Instruction,
  ...
  for (size_t Position = 0; Position < Markup.size(); ++Position) {
    ...
    if (Markup[Position] == '<') {
      ...
      Result.Tags.emplace_back(TagType, Result.Disassembled.size());
      Position += 4;
      ...
      yield::Tag &CurrentTag = Result.Tags[OpenTagStack.back()];
      ...
      size_t MnemonicStart = Result.Disassembled.size();
