...
tryConvertToCABI(const model::RawFunctionType &FunctionType,
  ...
  auto Arguments = C.tryConvertingRegisterArguments(FunctionType.Arguments());
  ...
  auto Stack = C.tryConvertingStackArguments(FunctionType.StackArgumentsType(),
                                             Arguments->size());
  if (!Stack.has_value()) {
  ...
  // And the return type.
  auto ReturnType = C.tryConvertingReturnValue(FunctionType.ReturnValues());
...
TCC::tryConvertingStackArguments(model::QualifiedType StackArgumentTypes,
  ...
  auto FirstAlignment = *ABI.alignment(Stack.Fields().begin()->Type());
  ...
  auto LastSize = LastArgument.Type().size();
