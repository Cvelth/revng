  ...
  if (!ReturnValue.Registers.empty()) {
    ...
    if (Function.ReturnType().Qualifiers().empty()) {
      ...
      if (Struct && Struct->Fields().size() == NewType.ReturnValues().size()) {
        ...
        std::uint64_t StructOffset = 0;
        for (std::size_t Index = 0; Index < Struct->Fields().size(); ++Index) {
    ...
    if (!Distributed.RepresentsPadding) {
      ...
      for (model::Register::Values Register : Distributed.Registers) {
        ...
        if (SimpleArgument) {
          ...
          auto ArgumentName = Function.Arguments().at(Index).name();
      ...
      if (Distributed.SizeOnStack != 0) {
        auto ArgumentIterator = Function.Arguments().find(Index);
        ...
        std::uint64_t Alignment = *ABI.alignment(Argument.Type());
        ...
        if (Distributed.Registers.empty()) {
          ...
          std::ptrdiff_t LeftoverSpace = (OccupiedSpace -= PointerSize * RC);
          while (LeftoverSpace > 0) {
            // TODO: figure out what to do with names.
            //       Since no two struct fields can have the same name, we
            //       cannot just carry them over. There's an option to make
            //       a bunch of nested structs to preserve these, but that
            //       opens another can of problems.
...
ToRawConverter::distributeOne(model::QualifiedType Type,
  ...
  revng_log(Log,
            "It's size is " << Size << " and it's alignment is " << Alignment
                            << ".");
  ...
  if (ABI.OnlyStartDoubleArgumentsFromAnEvenRegister()) {
    if (Size >= ABI.getPointerSize() && Alignment > ABI.getPointerSize()) {
...
ToRawConverter::distributeNonPositionBasedArguments(const ASet &Arguments,
  ...
  for (const model::Argument &Argument : Arguments) {
    ...
    *RegisterCounter = NextRegisterIndex;
    ...
    // All good - add the argument.
    auto Movable = llvm::make_range(std::make_move_iterator(Arguments.begin()),
                                    std::make_move_iterator(Arguments.end()));
    for (DistributedArgument &&Argument : Movable)
...
Layout::Layout(const model::CABIFunctionType &Function) {
  ...
  auto Distributed = Converter.distributeArguments(Function.Arguments(),
                                                   RV.SizeOnStack != 0);
  revng_assert(Distributed.size() >= Function.Arguments().size());
  std::size_t Index = 0;
  for (const DistributedArgument &Argument : Distributed) {
    if (!Argument.RepresentsPadding) {
      ...
      Current.Registers = std::move(Argument.Registers);
      if (Argument.SizeOnStack != 0) {
        ...
        std::uint64_t Alignment = *ABI.alignment(ArgumentType);
        if (CurrentStackOffset % Alignment != 0)
          CurrentStackOffset += (Alignment - CurrentStackOffset % Alignment);
        Current.Stack->Offset = CurrentStackOffset;

        // And carry the size over unchanged.
        Current.Stack->Size = Argument.SizeOnStack;
  ...
  FinalStackOffset = Converter.finalStackOffset(Distributed);
}
