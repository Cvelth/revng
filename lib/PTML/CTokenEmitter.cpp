...
void ptml::CTokenEmitter::emitUntypedIntegerLiteral(uint64_t Value) {
  // TODO: verify `Value` actually fits into the given `CIntegerKind`!
...
void ptml::CTokenEmitter::enterRegionImpl(ptml::Emitter::TagEmitter &Tag,
                                          RegionKind Kind,
                                          llvm::StringRef Location) {
  if (Location.empty())
    return;
  ...
  Tag.initializeOpenTag(PTML, ptml::tags::Span);
  Tag.emitAttribute(ptml::attributes::ActionContextLocation,
                    getActionContextLocation(Location));
  Tag.emitListAttribute(ptml::attributes::AllowedActions, Actions);
  Tag.finalizeOpenTag();
