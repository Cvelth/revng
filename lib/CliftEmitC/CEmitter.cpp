      ...
      case StackItemKind::Function: {
        ...
        if (F.getArgumentTypes().empty()) {
          ...
          for (auto [J, PT] : llvm::enumerate(F.getArgumentTypes())) {
            ...
            using CTERegion = ptml::CTokenEmitter::Region;
            auto Guard = [this, Declarator, J]() -> std::optional<CTERegion> {
              ...
              return std::make_optional<CTERegion>(Parent.Tokens,
                                                   RegionKind::Commentable,
                                                   CurrentLocation);
            }();
