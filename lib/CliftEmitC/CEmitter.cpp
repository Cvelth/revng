...
private:
  ...
  void emitConstIfNeeded(ValueType Type) {
    ...
    if (Type.isConst()) {
      Parent.PTML.emitKeyword(CTE::Keyword::Const);
      Parent.PTML.emitSpace();
    }
  }
  ...
  RecursiveCoroutine<void> emitImpl(ValueType Type,
    ...
    while (true) {
      ...
      if (auto T = mlir::dyn_cast<PrimitiveType>(Type)) {
        ...
        if (T.getPointerSize() == Parent.Target.PointerSize) {
          ...
          emitConstIfNeeded(T);
          Parent.PTML.emitLiteralIdentifier(Macro);
          Parent.PTML.emitPunctuator(CTE::Punctuator::LeftParenthesis);

          rc_recur DeclarationEmitter(Parent).emitImpl(T.getPointeeType(),
                                                       /*Declarator=*/nullptr);

          Parent.PTML.emitPunctuator(CTE::Punctuator::RightParenthesis);
          NeedSpace = true;
        }
        ...
        if (F and F == OutermostFunctionType) {
          ...
          emitConstIfNeeded(T);
          Parent.PTML.emitIdentifier(T.getName(),
                                     T.getHandle(),
                                     chooseEntityKind(T),
    ...
    for (auto [RI, SI] : llvm::enumerate(std::views::reverse(Stack))) {
      ...
      case StackItemKind::Pointer: {
        ...
        emitSpaceIfNeeded();
        Parent.PTML.emitPunctuator(CTE::Punctuator::Star);
        emitConstIfNeeded(T);
      } break;
    ...
    if (Declarator) {
      emitSpaceIfNeeded();
      Parent.PTML.emitIdentifier(Declarator->Identifier,
                                 Declarator->Location,
                                 Declarator->Kind,
    ...
    for (auto [I, SI] : llvm::enumerate(Stack)) {
      ...
      case StackItemKind::Array: {
        ...
        Parent.PTML.emitPunctuator(CTE::Punctuator::LeftBracket);
        ...
        Parent.PTML.emitIntegerLiteral(ExtentValue,
                                       CIntegerKind::Int,
                                       /*Radix=*/10);

        Parent.PTML.emitPunctuator(CTE::Punctuator::RightBracket);
      } break;
      case StackItemKind::Function: {
        ...
        if (F.getArgumentTypes().empty()) {
          ...
          for (auto [J, PT] : llvm::enumerate(F.getArgumentTypes())) {
            if (J != 0) {
              Parent.PTML.emitPunctuator(CTE::Punctuator::Comma);
              Parent.PTML.emitSpace();
            }
...
void CEmitter::emitPrimitiveType(clift::PrimitiveKind Kind, uint64_t Size) {
  if (Kind == PrimitiveKind::VoidKind) {
    revng_assert(Size == 0);
    PTML.emitKeyword(CTE::Keyword::Void);

  } else {
    PTML.emitPrimitive(getPrimitiveTypeCName(Kind, Size),
                       CTE::IdentifierKind::Reference);
  }
}
...
void CEmitter::emitAttribute(AttributeAttr Attribute) {
  ...
  PTML.emitSpace();
  PTML.emitIdentifier(Macro.getString(),
                      Macro.getHandle(),
  ...
  if (auto Arguments = Attribute.getArguments()) {
    ...
    for (auto [I, A] : llvm::enumerate(*Arguments)) {
      ...
      PTML.emitIdentifier(A.getString(),
                          A.getHandle(),
