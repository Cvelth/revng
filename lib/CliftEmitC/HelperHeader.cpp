...
static void printHelperPrototype(mlir::clift::CEmitter &Emitter,
  ...
  Emitter.emitComment("TODO: print " + Function.getHandle().str());
}

void mlir::clift::emitHelpers(ptml::CTokenEmitter &PTML,
                              const TargetCImplementation &Target,
                              const mlir::ModuleOp &Module) {
  TypeDefinitionEmitter Emitter(PTML, Target, {});

  PTML.emitComment("Types", ptml::CTokenEmitter::CommentKind::Category);
  PTML.emitNewline();

  // FUTURE-WIP: this might not be a good idea after all, look into this again
  // and possibly drop this.
