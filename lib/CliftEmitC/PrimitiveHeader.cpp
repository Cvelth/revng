...
static void emit16ByteGeneric(ptml::CTokenEmitter &PTML,
  ...
  auto Guard = PTML.enterScope(ptml::CTokenEmitter::ScopeKind::MacroIfDef, 0);
...
static void emitFakePrimitiveStruct(ptml::CTokenEmitter &PTML, size_t Size) {
  ...
  auto G = PTML.enterScope(ptml::CTokenEmitter::ScopeKind::StructDefinition);
  PTML.emitNewline();
  ...
  PTML.emitSimpleIntegerLiteral(Size);
...
static void emit10Or12ByteGeneric(ptml::CTokenEmitter &PTML, size_t Size) {
  auto Guard = PTML.enterScope(ptml::CTokenEmitter::ScopeKind::MacroIf, 0);
...
static void emit16ByteFloatImpl(ptml::CTokenEmitter &PTML) {
  ...
  PTML.emitOperator(ptml::CTokenEmitter::Operator::EqualsEquals);
  PTML.emitSpace();
  PTML.emitSimpleIntegerLiteral(16);
  PTML.emitNewline();
...
static void emit2ByteFloatImpl(ptml::CTokenEmitter &PTML) {
  ...
  PTML.emitOperator(ptml::CTokenEmitter::Operator::EqualsEquals);
  PTML.emitSpace();
  PTML.emitSimpleIntegerLiteral(1);
  PTML.emitSpace();
