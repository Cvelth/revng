...
void TypeDefinitionEmitter::emitComment(llvm::StringRef Content) {
  ...
  Comments.emitDoxygenComment(Content);
}
...
void TypeDefinitionEmitter::emitTypeDeclaration(mlir::clift::DefinedType Type) {
...
void TDEmitter::emitClassDefinition(mlir::clift::ClassType StructOrUnion) {
  {
    ...
    if (auto S = mlir::dyn_cast<mlir::clift::StructType>(StructOrUnion)) {
      ...
      emitAnnotation<"_SIZE">(S.getSize());
      Tokens.emitSpace();
    }
...

  {
    ...
    for (const auto &Field : StructOrUnion.getFields()) {
      ...
      if (IsStruct) {
        // TODO: find a cleaner way to determining whether a field has its
        //       default name or not. This implementation is very brittle!
        //
        // WARNING: this ignores `UnnamedStructFieldPrefix` configuration field,
        //          user might have set!
        model::CNameBuilder UnconfiguredNB(model::Binary{});
        model::StructDefinition FakeStruct; // No fields are read from it.
        model::StructField FakeField(Field.getOffset()); // Only `Offset` read.
        std::string AutomaticName = UnconfiguredNB.automaticName(FakeStruct,
                                                                 FakeField);
        if (AutomaticName != Field.getName()) {
          Tokens.emitSpace();
          emitAnnotation<"_STARTS_AT">(Field.getOffset());
        }
      }
...
void TypeDefinitionEmitter::emitEnumDefinition(mlir::clift::EnumType Enum) {
  {
    auto G = Tokens.enterRegion(ptml::CTokenEmitter::RegionKind::Commentable,
                                Enum.getHandle());
    ...
    Tokens.emitSpace();
    {
      auto AnnotationGuard = emitComplexAnnotation<"_ENUM_UNDERLYING">();
...
void TypeDefinitionEmitter::emitTypeDefinition(mlir::clift::DefinedType Type) {
...
void // formatting
TypeDefinitionEmitter::emitTypeTree(const mlir::clift::TypeDependencyNode &Root,
                                    NodeSet &Emitted) {
  ...
  for (const auto *Node : llvm::post_order_ext(&Root, Emitted)) {
    ...
    if (Node->IsDefinition) {
      ...
      revng_assert(mlir::clift::isSeparateDeclarationAllowed(Definition));
